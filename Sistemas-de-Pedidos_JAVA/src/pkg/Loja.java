...
public class Loja {
    ...
    public void CarregarDados() {
    	try {
            ...
            List<Pedido> pedidos = objectMapper.readValue(new File("clientes.json"),
                    new TypeReference<List<Pedido>>() {});
            this.pedidos.addAll(pedidos);
            
            for (int i=0; i<this.fornecedores.size(); i++) {
            	for (int j=0; j<this.fornecedores.get(i).getProdutos().size();j++) {
            		this.fornecedores.get(i).getProdutos().get(j).setFornecedor(this.fornecedores.get(i));
            		//por conta da importação circular, precisa adicionar o fornecedor do item manualmente
            		//por isso tem o JsonIgnore no produtos.fornecedor
            	}
            }
            
            for (int i=0; i<this.clientes.size();i++) {
            	for (int j=0; i<this.clientes.get(i).getPedidos().size();j++) {
            		this.clientes.get(j).getPedidos().get(j).setCliente(this.clientes.get(i));
            	}
            }
